...
def xyzcube(face_w, overlap=1.0):
    '''
    Return the xyz cordinates of the unit cube in [F R B L U D] format.
    '''
    
    out = np.zeros((face_w, int(face_w*overlap)* 6, 3), np.float32)
    x_rng = np.linspace(-0.5, 0.5, num=face_w, dtype=np.float32)  
    y_rng = np.linspace(-0.5*overlap, 0.5*overlap, num=int(face_w*overlap), dtype=np.float32)  
    grid = np.stack(np.meshgrid(y_rng, -x_rng), -1)

    # Front face (z = 0.5)
    face_w = int(face_w *overlap)
...
def cube_h2list_overlap(cube_h, overlap=1.5):
    # assert cube_h.shape[0] * 6 == cube_h.shape[1]
    return np.split(cube_h, 6, axis=1)
################################################
...
################################################
# Revised here by THKIM
def cube_h2dict_overlap(cube_h):
    cube_list = cube_h2list_overlap(cube_h, overlap=1.5)
    return dict([(k, cube_list[i])
                 for i, k in enumerate(['F', 'R', 'B', 'L', 'U', 'D'])])
################################################
...
################################################
# Revised here by THKIM
def cube_h2dice_overlap(cube_h, overlap=1.5):
    # overlap: if you want to overlap two cubemap's faces more, revise here.
    face_k=['F', 'R', 'B', 'L', 'U', 'D']
    ...
    for i, (sx, sy) in enumerate(sxy):
        ...
        # # Output image
        # Image.fromarray(face.astype(np.uint8)).save(face_k[i]+".png")
        separated_cube.append(face.astype(np.uint8))
    return cube_dice, separated_cube
################################################
...
################################################
# Revised here by THKIM
def cube_dice2h_overlap(cube_dice, overlap=1.5):
    w = cube_dice.shape[0] // 3
    assert cube_dice.shape[0] == w * 3 and cube_dice.shape[1] == w * 4 * overlap
    cube_h = np.zeros((w, w * 6, cube_dice.shape[2]), dtype=cube_dice.dtype)
    # Order: F R B L U D
    sxy = [(1, 1), (2, 1), (3, 1), (0, 1), (1, 0), (1, 2)]
    for i, (sx, sy) in enumerate(sxy):
        face = cube_dice[sy*w:(sy+1)*w, sx*int(w*overlap)+int(w*(overlap-1)/2): sx*int(w*overlap)+int(w*(overlap-1)/2)+w]
        if i in [1, 2]:
            face = np.flip(face, axis=1)
        if i == 4:
            face = np.flip(face, axis=0)
